...
function createRunlistWebviewRouter(host, adapters = {}) {
  ...
  const route = createWebviewCommandRouter({
    handlers: {
      ...
      openPhoneHandoff: (message) => host.openPhoneHandoff(message.id),
      changePhoneHandoffNetwork: (message) => host.changePhoneHandoffNetwork(message.id),
      openWorkspaceFolder: () => host.openWorkspaceFolder(),
      showTerminal: (message) => host.showProjectTerminal(message.id),
