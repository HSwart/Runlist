...
| **Preview app** | A chevron appears at the far right when any configured web service responds, even while another service is still starting. Runlist uses the first responding web service in the saved order. Expand it to load one live preview beneath the project, then choose Desktop or a phone-sized Mobile view. Both use the same local service; collapsing or losing that service unloads the preview. Some apps block embedded views, so Open in browser remains available. |
| **Resource use** | Shows current CPU and memory with a short live pulse inside an expanded preview when this VS Code window started the project. The pulse is kept only while the preview is open; Runlist stops measuring when it closes or ownership is uncertain. |
| **Recent starts** | Shows up to five completed Runlist-managed starts in an expanded project card, including whether each became ready, failed, or was still starting after the readiness timeout. Only the outcome, completion time, and startup duration are retained. |
