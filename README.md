...
| **Recent starts** | Shows up to five completed Runlist-managed starts in History, including whether each became ready, failed, or was still starting after the readiness timeout. Select a failed start when available to see its short, sanitized failure reason; full logs remain in Recent Output. |
