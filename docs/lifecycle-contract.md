...
| --- | --- | --- |
| Start, Restart, Stop | New generations replace old ones; the old tree is gone; final Stop clears coordination | Native lifecycle smoke |
| Root exits with descendants | Descendants are verified and stopped before ownership is released | `project-process.test.js`, native lifecycle smoke |
| Shell performs `exec` | Launch identity remains valid for the owned supervisor/tree | `project-process.test.js`, native lifecycle smoke |
| PID or host PID is reused | No signal or stale-lock refresh occurs for the replacement | `project-process.test.js`, `port-gate.test.js`, `project-store.test.js` |
...
| Two VS Code windows compete | One generation wins; stale cleanup cannot release replacement state | `project-process.test.js`, `port-gate.test.js`, `project-restart.test.js` |
...
| External listener changes after confirmation | Operation aborts without signaling either process | `port-recovery.test.js`, `port-process.test.js` |
...
| Custom Stop fails or cannot prove completion | Runlist reports failure and retains uncertain coordination | `project-restart.test.js`, `custom-stop-recovery.test.js` |
| Partial storage or lock update | State is recovered or retained fail-closed without deleting newer data | `project-store.test.js`, `project-process.test.js`, `port-gate.test.js` |
