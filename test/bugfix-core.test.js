...
test('delete while starting cancels the start before removing the saved project', () => {
  ...
  const cancelBlock = source.slice(cancelStart, cancelStart + 450);
...
test('compose start block keeps Show terminal primary while not-responding', () => {
  const action = projectPrimaryAction({
    name: 'Api',
  ...
  assert.equal(action.action, 'show-terminal');
  assert.equal(action.mode, 'terminal');
});
