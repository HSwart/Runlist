...
test('reload honesty reports Running before the first status poll when the process is still owned', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'runlist-reload-honesty-'));
  ...
  const Provider = loadRunlistProvider(() => undefined, []);
  const provider = Object.create(Provider.prototype);
  provider.processOwnership = reloaded;
  provider.portReservations = { adoptAbandonedLiveReservations: () => [] };
  provider.managedProjectIds = new Set();
  provider.projectStatuses = new Map();
  provider.stoppingProjectIds = new Set();
  provider.processes = new Map();
  provider.startAttempts = new Map();

  assert.deepEqual(provider.recoverLiveRuntimeAfterReload(), ['project-1']);
  assert.equal(provider.getProjectStatus('project-1'), 'running');
...
test('reload honesty recovers live ownership after identity reconcile', () => {
  const source = readShippedHostSource();
  const constructorRecover = source.indexOf('this.recoverLiveRuntimeAfterReload();');
  const startMonitoring = source.indexOf('startStatusMonitoring() {');
  const startRecover = source.indexOf('this.recoverLiveRuntimeAfterReload();', startMonitoring);
  const refresh = source.indexOf('async refreshProjectStatuses()');
  const reconcile = source.indexOf('this.processOwnership.reconcileProcessIdentities()', refresh);
  const releaseDead = source.indexOf('this.releaseDeadOwnedRuntime()', reconcile);
  const refreshRecover = source.indexOf('this.recoverLiveRuntimeAfterReload()', reconcile);
  assert.ok(constructorRecover >= 0);
  assert.ok(startMonitoring < startRecover);
  assert.ok(refresh < reconcile);
  assert.ok(reconcile < releaseDead);
  assert.equal(refreshRecover, -1);
  assert.match(source, /this\.processes\.has\(id\) \|\| this\.processOwnership\?\.isCurrentOwner\?\.\(id\)/);
  assert.match(source, /this\.recoverLiveRuntimeAfterReload\(\);[\s\S]*this\.refreshProjectStatuses\(\)/);
});
