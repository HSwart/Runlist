...
test('classifies only completed managed-start outcomes and stores no extra details', () => {
...
test('wires bounded outcomes and an accessible non-color-only ribbon into the lifecycle', () => {
  ...
  assert.match(extension, /showStartFailure\(project, details[\s\S]*recordStartupOutcome\(project\.id, 'failed'\)/);
  assert.match(extension, /const startFailed = startExitFailed[\s\S]*recordStartupOutcome\(id, 'failed'\)[\s\S]*this\.processOwnership\.release\(id\)/);
  ...
  assert.match(webview, /class="startup-history-ribbon"[\s\S]*aria-hidden="true"/);
