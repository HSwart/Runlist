...
const { readProjectDiagnostics } = require('../src/projects/project-diagnostics');
...
async function run() {
  ...
  assert.equal(
    ...
    false,
    'Start should block when a required key is missing.'
  );
  assertRequiredEnvFailure(api, missingRequired.id, /Missing: API_KEY/);
  ...
  assert.equal(
    ...
    false,
    'Start should block when a required key is empty in the attached env file.'
  );
  assertRequiredEnvFailure(api, emptyRequired.id, /Empty in \.env\.local: API_KEY/);
...
function assertRequiredEnvFailure(api, projectId, detailPattern) {
  const summary = api.provider.projectFailureSummaries.get(projectId);
  assert.ok(summary, 'Start failure summary was not recorded.');
  assert.equal(summary.kind, 'missing-required-env');
  const diagnostics = readProjectDiagnostics(api.projectsFile, projectId);
  assert.equal(diagnostics?.failureSummary?.kind, 'missing-required-env');
  const stored = api.provider.projectFailureDetails.get(projectId);
  const detail = typeof stored?.detail === 'string'
    ? stored.detail
    : (summary.message || '');
  assert.match(detail, detailPattern);
}
